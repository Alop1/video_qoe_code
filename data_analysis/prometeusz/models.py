...
class Report(object):
    ...
    def __init__(self):
        self.report_file = open("devnull.txt", "a+")
        path = "models_summary_smrLinear_svrLinear_4HL_with_res.csv"
...
def support_vector_regresion_model(training_data, testing_data, *features):
    ...
    x_test = testing_data[list(features)]
    y_test = testing_data["Subject_score"]
    svr = svm.SVR(kernel="linear", C=1.2)
...
def nn_model(training_data, testing_data, *features):
    ...
    nn = MLPRegressor(hidden_layer_sizes=(13,13,13,13), max_iter=10000) # max_iter=500 random_state=9
